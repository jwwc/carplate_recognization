...
class LPR():
    ...
    def to_refine(self,image, pts, scale=3.0):
        ...
        mat_ = cv2.estimateAffinePartial2D(org_pts,target_pts,np.zeros((2,3)))
        dsize = (int(120 * scale), int(48 * scale))
        warped = cv2.warpAffine(image, mat_[0], dsize)
        return warped
    ...
    def plate_recognition(self,image,minSize=30,charSelectionDeskew=True,mode='ssd'):
        ...
        # for results,confidencet,(left,top,right,bottom) in res_set:
                cv2.rectangle(image,(left,top-20),(right,bottom+20),(0,0,255),2)
            # font = cv2.FONT_HERSHEY_SIMPLEX
            # print(type(image),"===========++++++++++")
            # if(image.any() != None):
            # cv2.putText(image,results,(left,bottom-15),font,0.75,(255,255,255),2)
            # image = self.cv2ImgAddText(image, results,10,65)
        return image
